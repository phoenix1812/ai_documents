...
class Database:
    ...
    def get_by_file_hash(
        self,
        file_hash: str,
        exclude_paperless_id: int | None = None,
    ) -> dict[str, Any] | None:
        params: list[Any] = [file_hash]
        where = "file_hash = ?"

        if exclude_paperless_id is not None:
            where += " AND (paperless_id IS NULL OR paperless_id != ?)"
            params.append(exclude_paperless_id)
